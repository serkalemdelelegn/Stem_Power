...
export default function IncubationPage() {
  ...
  >([]);
  const [incubationPhases, setIncubationPhases] = useState<any[]>([]);
  ...
  const [translatedContent, setTranslatedContent] = useState<{
    ...
    aboutOurIncubationProgram?: string;
    nurturingIdeasIntoImpactfulVentures?: string;
    aboutDescription?: string;
    whyChooseOurIncubationProgram?: string;
    whyChooseDescription?: string;
    provenTrackRecord?: string;
    accessToFabLabFacilities?: string;
    strongNetwork?: string;
    personalizedMentorship?: string;
    duration?: string;
    businessPlanningStrategy?: string;
    businessPlanningDescription?: string;
    prototypingDevelopment?: string;
    prototypingDescription?: string;
    mentorshipCoaching?: string;
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [hero, stats, phases, appPrograms, stories] = await Promise.all([
          fetchHero(),
          fetchStatistics(),
          fetchIncubationPhases(),
          fetchApplicationPrograms(),
        ...
        if (isMounted) {
          ...
          if (stats && stats.length > 0) {
            ...
            const mappedStats = stats.map((stat: ImpactStat) => {
              const iconKey = stat.icon?.toLowerCase() || "rocket";
              const IconComponent = iconMap[iconKey] || Rocket;
              return {
                number: stat.number,
                label: stat.label,
                icon: IconComponent,
          ...
          if (stories && stories.length > 0) {
            ...
            const mappedStories = stories.map((story: SuccessStory) => ({
              id: story.id,
              name: story.name,
              contactPerson: story.contactPerson,
              phone: story.phone,
              email: story.email,
  ...
  useEffect(() => {
    ...
    const translateDynamicContent = async (targetLang: "en" | "am") => {
      ...
      try {
        ...
        // Translate incubation phases
        if (incubationPhases && incubationPhases.length > 0) {
          translations.phases = await Promise.all(
            incubationPhases.map(async (phase) => ({
              title: phase.title
                ? await translateText(phase.title, targetLang)
                : phase.title,
              description: phase.description
                ? await translateText(phase.description, targetLang)
                : phase.description,
              duration: phase.duration
                ? await translateText(phase.duration, targetLang)
                : phase.duration,
              badge: phase.badge
                ? await translateText(phase.badge, targetLang)
                : phase.badge,
            }))
          );
        }
        ...
        translations.aboutOurIncubationProgram = await translateText(
          "About Our Incubation Program",
          targetLang
        );
        translations.nurturingIdeasIntoImpactfulVentures = await translateText(
          "Nurturing Ideas into Impactful Ventures",
          targetLang
        );
        translations.aboutDescription = await translateText(
          "Our Incubation programs provide startups and innovators with the guidance, resources, and mentorship they need to turn bold ideas into successful, sustainable ventures. From business planning and prototyping to access to funding, networks, and collaborative workspaces, we create an environment where creativity and entrepreneurship thrive.",
          targetLang
        );
        translations.whyChooseOurIncubationProgram = await translateText(
          "Why Choose Our Incubation Program?",
          targetLang
        );
        translations.whyChooseDescription = await translateText(
          "We provide comprehensive support across all aspects of building and scaling your venture, ensuring you have the expertise and resources needed at every turn.",
          targetLang
        );
        ...
        translations.accessToFabLabFacilities = await translateText(
          "Access to state-of-the-art FabLab facilities",
          targetLang
        );
        translations.strongNetwork = await translateText(
          "Strong network of investors and partners",
          targetLang
        );
        translations.personalizedMentorship = await translateText(
          "Personalized mentorship from industry experts",
          targetLang
        );
        translations.duration = await translateText("Duration:", targetLang);
        translations.businessPlanningStrategy = await translateText(
          "Business Planning & Strategy",
          targetLang
        );
        translations.businessPlanningDescription = await translateText(
          "Develop comprehensive business plans, market analysis, and go-to-market strategies with expert guidance.",
          targetLang
        );
        translations.prototypingDevelopment = await translateText(
          "Prototyping & Development",
          targetLang
        );
        translations.prototypingDescription = await translateText(
          "Access state-of-the-art facilities to transform ideas into tangible prototypes and MVPs.",
          targetLang
        );
        translations.mentorshipCoaching = await translateText(
          "Mentorship & Coaching",
          targetLang
        );
        translations.mentorshipDescription = await translateText(
          "Connect with successful entrepreneurs and industry experts for personalized guidance.",
          targetLang
        );
        translations.fundingAccess = await translateText(
          "Funding Access",
          targetLang
        );
        translations.fundingDescription = await translateText(
          "Get connected to investors and navigate the fundraising process with confidence.",
          targetLang
        );
        ...
        translations.applicationProgramsDescription = await translateText(
          "All our incubation programs are completely free. Apply now to join our next cohort and transform your idea into reality.",
          targetLang
        );
        ...
        translations.successStoriesDescription = await translateText(
          "Meet the innovative entrepreneurs who have grown through our incubation program and are now transforming industries and improving lives across Ethiopia.",
          targetLang
        );
  ...
  const displayStats =
    translatedContent.statistics && selectedLanguage !== "en"
      ...
      : impactStats;
  const displayPhases =
    translatedContent.phases && selectedLanguage !== "en"
      ? incubationPhases.map((phase, index) => ({
          ...phase,
          title: translatedContent.phases![index]?.title || phase.title,
          description:
            translatedContent.phases![index]?.description || phase.description,
          duration:
            translatedContent.phases![index]?.duration || phase.duration,
          badge: translatedContent.phases![index]?.badge || phase.badge,
        }))
      : incubationPhases;
      ...
      <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-emerald-50/30">
        ...
        <section className="max-w-6xl mx-auto px-4 py-16">
          ...
          <div className="text-center mb-12">
            ...
            <h2 className="text-4xl md:text-5xl mb-6 bg-linear-to-br from-[#367375] to-[#24C3BC] text-transparent bg-clip-text font-bold">
              {getTranslatedText(
                "nurturingIdeasIntoImpactfulVentures",
                "Nurturing Ideas into Impactful Ventures"
              )}
            </h2>
          ...
          {displayPhases.length > 0 ? (
            <div className="grid md:grid-cols-2 gap-6 mb-12">
              {displayPhases
                ...
                .map((phase: IncubationPhase) => {
                  const iconKey =
                    typeof phase.icon === "string"
                      ? phase.icon.toLowerCase()
                      : "fileText";
                  const PhaseIcon =
                    phaseIconMap[iconKey as keyof typeof phaseIconMap] ||
                    FileText;
                  return (
                    <Card
                      key={phase.id}
                    ...
                    >
                      ...
                      <CardContent className="pt-8 pb-6">
                        ...
                        <p className="text-sm text-[#367375] font-medium mb-3">
                          {getTranslatedText("duration", "Duration:")}{" "}
                          {phase.duration}
                        </p>
            ...
            <Card className="mb-12 border-2">
              <CardContent className="pt-12 pb-12 px-6 text-center">
                ...
                <p className="text-muted-foreground text-lg">
                  Incubation phases will be displayed here once they are added.
                </p>
              </CardContent>
            </Card>
          )}
                    ...
                    >
                      <div className="relative h-48">
                        ...
                        <div className="absolute top-4 right-4">
                          <Badge className="bg-linear-to-br from-[#367375] to-[#24C3BC] text-white border-white">
                            {program.status}
                          </Badge>
                        </div>
                      </div>
        ...
        <section className="max-w-6xl mx-auto px-4 py-16">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-muted-foreground max-w-3xl mx-auto text-pretty">
              Meet the innovative entrepreneurs who have grown through our
              incubation program and are now transforming industries and
              improving lives across Ethiopia.
            </p>
          </div>
          ...
          {displaySuccessStories.length > 0 ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-2.5 mb-8">
              {displaySuccessStories.map((business, index) => (
                <Card
