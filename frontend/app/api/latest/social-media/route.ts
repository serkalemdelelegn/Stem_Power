...
export async function GET(req: NextRequest) {
  try {
    const response = await backendApiServer.get("/api/news/social-media", req);
    const postsArray = Array.isArray(response) ? response : [];

    // Transform posts
    const transformedPosts = postsArray
      .map(transformBackendPostToFrontend)
