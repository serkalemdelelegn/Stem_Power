...
interface ProgramDynamicPagesProps {
  program: string;
  programLabel: string;
  description?: string;
}

export function ProgramDynamicPages({
  program,
  programLabel,
  description,
}: ProgramDynamicPagesProps) {
  ...
  const fetchPages = async () => {
    try {
      ...
      const data = await backendApi.get(
        `/api/pages?program=${encodeURIComponent(program)}`
      );
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {description || `Create and manage ${programLabel} dynamic pages.`}
          </p>
        </div>
        <Button
        ...
        <Card className="text-center py-12">
          <CardContent>
            ...
            <p className="text-gray-600 mb-4">
              Create your first {programLabel} dynamic page to get started
            </p>
      ...
      >
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingPage ? "Edit Page" : `Create ${programLabel} Page`}
            </DialogTitle>
            <DialogDescription>
